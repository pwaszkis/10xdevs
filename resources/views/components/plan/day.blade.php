@props(['day', 'expanded' => false])

@php
    use App\DataTransferObjects\PlanDayViewModel;
    $dayViewModel = PlanDayViewModel::fromArray($day);
    $pointsByDayPart = $dayViewModel->getPointsByDayPart();
                ...
                @if(isset($pointsByDayPart[$dayPart]))
                    <div class="day-part-section mb-6 last:mb-0">
                        <h4 class="text-lg font-medium text-gray-700 mb-3 flex items-center gap-2">
                            ...
                            <span>{{ $dayPartLabels[$dayPart] ?? 'Inne' }}</span>
                        </h4>

                        <div class="space-y-3">
                            @foreach($pointsByDayPart[$dayPart] as $point)
                                <x-plan.point :point="$point" />
                            @endforeach
                        </div>
                    </div>
                @endif
